...
#showruns#show PATCH  /showblogs/:showblog_id/showruns/:id(.:format)
 ...
 def update 
   showblog = Showblog.find(params[:showblog_id])
  @showrun = showblog.showruns.find(params[:id])
   if @showrun.update(params.require(:showrun).permit(:showblog_id, :player, :event, :spec_vote, :level, :make_other, :note_showrun, showun_attributes: [:showblog_id, :player, :event, :spec_vote, :level, :make_other, :note_showrun] ))
